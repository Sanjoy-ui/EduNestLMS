...
function SignUp() {
  ...
  const handleSignUp = async () => {
    setLoading(true)
    try {
      const result = await axios.post(serverUrl + "/api/v1/auth/signup", { name, email, password, role }, { withCredentials: true })
  ...
  const googleSignUp = async () => {
    try {
      ...
      const result = await axios.post(serverUrl + "/api/v1/auth/googlesignup", { name, email, role: role || "student" }, { withCredentials: true })
      dispatch(setUserData(result.data))
      navigate("/")
      toast.success("SignUp Successfully")
    } catch (error) {
          ...
          <div className='inline-flex items-center gap-3 bg-[#060a17] border border-white/15 rounded-2xl px-5 py-3 shadow-2xl mb-4'>
            <div className="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-gradient-to-br from-cyan-500/20 via-indigo-500/20 to-purple-500/20 border border-cyan-500/30 rounded-xl">
              <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M4 20V4L14 16V4M14 16L20 4V20" stroke="url(#signupNexusGrad)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                <defs>
                  <linearGradient id="signupNexusGrad" x1="4" y1="4" x2="20" y2="20" gradientUnits="userSpaceOnUse">
